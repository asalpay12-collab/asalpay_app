...
import '../filter/filteredTransactionsTwo.dart';

class FilterScreenTwo extends StatefulWidget {
  final String wallet_accounts_id;

  const FilterScreenTwo({super.key, required this.wallet_accounts_id});

  @override
  _FilterScreenTwoState createState() => _FilterScreenTwoState();
}

class _FilterScreenTwoState extends State<FilterScreenTwo> {
...
void _applyFilters(BuildContext context) async {
  if (_chosenType == null || _dateRange == null) {
    showDialog(
      context: context,
      builder: (BuildContext context) {
        return AlertDialog(
          title: const Text('Missing Information'),
          content: const Text('Please select both the type and date range.'),
          actions: [
            TextButton(
              onPressed: () {
  ...
  final String walletId = widget.wallet_accounts_id; 
  ...
  try {
    ...
    if (_chosenType == "Remittance") {
      allTransactions = await homeSliderAndTransaction.fetchAndSetAllTrRemittance(
        walletId: walletId,
        startDate: _dateRange?.start.toString() ?? "",
        endDate: _dateRange?.end.toString() ?? "",
      );
    } else if (_chosenType == "Wallet") {
      await homeSliderAndTransaction.fetchAndSetWalletTransactions(
        walletId: walletId,
        startDate: _dateRange?.start.toString() ?? "",
        endDate: _dateRange?.end.toString() ?? "",
      );
      
      allTransactions = homeSliderAndTransaction.walletTransactions;
    }

    print("All Transactions found are here \n");
    print(allTransactions);

    Navigator.push(
      context,
      MaterialPageRoute(
        builder: (context) => FilteredTransactionsTwo(
          data: {
            'filters': {
              ...
              // 22/05/24
              'phoneNumber': _filterByCustomer ? _phoneController.text : null,
              // 22/05/24
          ...
          walletId: walletId,
          handleTypeSelection: _handleTypeSelection, 
        ),
      ),
    );
  } catch (error) {
...
Widget _buildFilterCard(String title, String value) {
  ...
  void initState() {
    super.initState();
    WidgetsBinding.instance.addPostFrameCallback((_) => _showTypeSelection());
  }
}
