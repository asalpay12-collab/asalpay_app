...
/// E-Wareeji Sell – wallets from API; rate soo la akhriyo; confirm → create transaction (type SELL).
class EwareejiSellScreen extends StatefulWidget {
  const EwareejiSellScreen({super.key, this.wallet_accounts_id});
...
class _EwareejiSellScreenState extends State<EwareejiSellScreen> {
  ...
  /// Company wallet address for the selected customer wallet (shown on success step).
  String? _companyWalletAddress;
  ...
  Future<void> _fetchRateIfNeeded() async {
    ...
    final rate = await EwareejiApiService.instance
        .getRateForAmount(_selectedCustomerWallet!.walletId, amount, type: 'sell');
    if (!mounted) return;
    final rateVal = rate?.rate ?? rate?.sellRate;
    setState(() {
      _selectedRate = rate;
      _loadingRate = false;
      if (rateVal != null && rateVal > 0)
        _rateController.text = rateVal.toStringAsFixed(4);
      else
        _rateController.text = 'No rate';
    });
  }
  ...
  Future<void> _openSetupAndReload() async {
    await Navigator.push(
      context,
      MaterialPageRoute(
        builder: (_) => EwareejiSetupScreen(wallet_accounts_id: widget.wallet_accounts_id),
      ),
    );
    if (!mounted) return;
    _loadWallets();
    ...
    return Center(
      child: Padding(
        ...
        child: Column(
          ...
          children: [
            Icon(Icons.account_balance_wallet_outlined, size: 72, color: Colors.white.withOpacity(0.9)),
  ...
  Widget _buildForm() {
    return Column(
      ...
      children: [
          ...
          _loadingWallets
              ...
              : _customerWallets.isEmpty
                  ...
                  : DropdownButtonFormField<int>(
                      value: _customerWallets.any((w) => w.id == (_selectedCustomerWallet?.id))
                      ...
                      onChanged: (v) {
                        ...
                        setState(() {
                          _selectedCustomerWallet = found.isNotEmpty ? found.first : _selectedCustomerWallet;
                          _selectedRate = null;
                          _rateController.clear();
                        });
        ...
        _sectionCard(
            'Crypto amount (you sell)',
            TextFormField(
              controller: _amountController,
              keyboardType:
                  const TextInputType.numberWithOptions(decimal: true),
              decoration: _inputDecoration(hint: 'Crypto to sell'),
            )),
        const SizedBox(height: 16),
        _sectionCard(
          'Rate',
          _loadingRate
              ...
              : TextFormField(
                  ...
                  decoration: _inputDecoration().copyWith(
                    ...
                    suffixIcon: Icon(Icons.trending_down_rounded,
        ...
        Builder(
          builder: (context) {
            final crypto = double.tryParse(_amountController.text.trim()) ?? 0;
            final rate = _selectedRate?.rate ?? _selectedRate?.sellRate ?? 0;
            final fiat = rate > 0 ? crypto * rate : 0.0;
            final hasFiat = rate > 0 && fiat > 0;
            return Container(
              padding: const EdgeInsets.all(16),
              decoration: BoxDecoration(
              ...
              child: Row(
                children: [
                  Icon(Icons.attach_money, color: secondryColor.withOpacity(0.9), size: 28),
                  const SizedBox(width: 14),
                  Expanded(
                    child: Column(
                      crossAxisAlignment: CrossAxisAlignment.start,
                      children: [
                        Text('Fiat amount (you receive)', style: TextStyle(fontSize: 12, color: Colors.white.withOpacity(0.85))),
                        const SizedBox(height: 4),
                        Text(
                          hasFiat ? '\$${fiat.toStringAsFixed(2)}' : '—',
        ...
        if (_showNegotiate) ...[
          ...
          _sectionCard(
              'Negotiate',
              const Text(
                  'Request a custom rate from the buyer. (Sample: Negotiate feature)',
          ...
          children: [
            Expanded(
              child: OutlinedButton(
                onPressed: () =>
                    setState(() => _showNegotiate = !_showNegotiate),
  ...
  void _copyAddress(BuildContext context) {
    final address = _companyWalletAddress ?? '';
  ...
  Widget _buildDetails() {
    ...
    final cryptoAmount = double.tryParse(amountStr) ?? 0;
    final rate = _selectedRate?.rate ??
        _selectedRate?.sellRate ??
        double.tryParse(_rateController.text) ??
        0;
    final fiatAmount = rate > 0 ? cryptoAmount * rate : 0;
    return Column(
      crossAxisAlignment: CrossAxisAlignment.stretch,
      children: [
        _infoRow('Wallet', _selectedCustomerWallet?.displayLabel ?? ''),
        _infoRow('Rate', rate > 0 ? rate.toStringAsFixed(4) : 'No rate'),
        _infoRow('Crypto amount (you sell)', cryptoAmount > 0 ? cryptoAmount.toStringAsFixed(8) : '—'),
        _infoRow('Fiat amount (you receive)', fiatAmount > 0 ? '\$${fiatAmount.toStringAsFixed(2)}' : '—'),
        if (rate > 0 && cryptoAmount > 0)
          Padding(
            padding: const EdgeInsets.only(top: 4),
            child: Text(
              '${cryptoAmount.toStringAsFixed(8)} × ${rate.toStringAsFixed(4)} = \$${fiatAmount.toStringAsFixed(2)}',
  ...
  Widget _addressRow(BuildContext context) {
    final address = _companyWalletAddress?.trim() ?? '';
    return Container(
        ...
        children: [
          Expanded(
            child: Column(
              ...
              children: [
                Text(
                  'Company address (send crypto here)',
                  style: TextStyle(
  ...
  Future<void> _onApproveSell() async {
    final cryptoAmount = double.tryParse(_amountController.text.trim()) ?? 0;
    final rate = _selectedRate?.rate ?? _selectedRate?.sellRate ?? 0;
    final fiatAmount = rate > 0 ? cryptoAmount * rate : 0;
    if (cryptoAmount <= 0 || rate <= 0 || _selectedCustomerWallet == null) return;
    final walletAccountId = int.tryParse(widget.wallet_accounts_id ?? '0') ?? 0;
    if (walletAccountId <= 0 && mounted) {
      ScaffoldMessenger.of(context).showSnackBar(
        const SnackBar(content: Text('Wallet account not set. Open E-Wareeji from home.'), duration: Duration(seconds: 4)),
      );
      return;
    }
    final svc = EwareejiApiService.instance;
    final companyWallets = await svc.getCompanyWallets(activeOnly: true);
    final compList = companyWallets.where((w) => w.walletId == _selectedCustomerWallet!.walletId).toList();
    final companyWallet = compList.isEmpty ? null : compList.first;
    if (companyWallet == null && mounted) {
      ScaffoldMessenger.of(context).showSnackBar(
        SnackBar(content: Text(svc.lastError ?? 'No company wallet for this network.'), duration: const Duration(seconds: 4)),
      );
      return;
    }
    if (!mounted) return;
    ...
    setState(() {
      _submitting = false;
      if (txId != null) {
        _companyWalletAddress = companyWallet.address;
        _step = 3;
      }
    });
    if (txId == null && mounted) {
      ScaffoldMessenger.of(context).showSnackBar(
        SnackBar(content: Text(svc.lastError ?? 'Failed to create transaction'), duration: const Duration(seconds: 5)),
      );
    }
  }
    ...
    return Column(
      ...
      children: [
        ...
        SizedBox(
          height: 52,
          child: ElevatedButton(
            onPressed: _submitting ? null : _onApproveSell,
            style: ElevatedButton.styleFrom(
                backgroundColor: Colors.white,
                foregroundColor: secondryColor,
                shape: RoundedRectangleBorder(
                    borderRadius: BorderRadius.circular(14))),
            child: _submitting
                ? const SizedBox(height: 24, width: 24, child: CircularProgressIndicator(strokeWidth: 2, color: secondryColor))
                : const Text('Approve'),
          ),
        ),
      ],
  ...
  Widget _buildConfirmation() {
    ...
    return Column(
      ...
      children: [
        ...
        Center(
            child: Text(
                'Crypto: ${cryptoAmount.toStringAsFixed(8)} × Rate: ${rate.toStringAsFixed(4)} = Fiat: \$${fiatAmount.toStringAsFixed(2)}',
                textAlign: TextAlign.center,
                style: TextStyle(
